...
async def get_top_agents(limit: int = 5):
    db = get_db_session()
    try:
        # Removed tier and team columns; use agent_type and default 'General'
        result = db.execute(text("""
...
async def get_agent_details(agent_id: str):
    ...
    try:
        ...
        return {
            ...
            "role": "agent",  # default role, removed from schema
...
async def get_online_agents():
    if REDIS_AVAILABLE and redis_client:
        try:
            ...
            details = []
            for aid in online:
                d = redis_client.hgetall(f"agent:{aid}")
                details.append({"agent_id": aid, "full_name": d.get("full_name", aid), "status": d.get("status", "Unknown"), "login_time": d.get("login_time")})
...
def generate_callcenter_xml():
    ...
    try:
        ...
        for q in queues:
            lines.append(f'    <queue name="{q.name}@default">')
        ...
        for a in agents:
            lines.append(f'    <agent name="{a.agent_id}@default">')
            lines.append('      <param name="type" value="callback"/>')
            lines.append(f'      <param name="contact" value="user/{a.extension}"/>')
            lines.append(f'      <param name="status" value="{a.status or "Available"}"/>')
        ...
        if tiers:
            lines.append('  <tiers>')
            for t in tiers:
                lines.append(f'    <tier queue="{t.queue_name}@default" agent="{t.agent_id}@default" level="1"/>')
            lines.append('  </tiers>')
        ...
        with open(path, "w") as f:
            f.write(xml)
        app_logger.info("Generated callcenter.conf.xml")
        return True
    except Exception as e:
...
# ============================================
# Routers (exported as required by main.py)
# ============================================

# 1. Queue / Campaign router -> named "router"
...
# 4. Agent management router (updated schema)
agents_router = APIRouter(prefix="/admin/agents", tags=["agents"])

class AgentBase(BaseModel):
    agent_id: str
    agent_identity: str = ""          # new field
    full_name: str
    extension: str
    status: str = "LoggedOut"
    sip_password: Optional[str] = None
    agent_type: str = "Normal"        # enum: Normal, Callback, Barge
...
# Campaign assignment endpoints (still valid, they use campaign_agents table)
