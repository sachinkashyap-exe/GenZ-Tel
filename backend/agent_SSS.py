...
def originate_call_direct(agent_id: str, phone_number: str, caller_id: str = None) -> dict:
    ...
    try:
        ...
        # 3. Normalise phone number (strip leading zero)
        original_number = phone_number
        if phone_number.startswith('0'):
            phone_number = phone_number[1:]

        # 4. Build originate command EXACTLY like the working CLI command
        #    We use sofia/internal/ + the contact_uri (which already is a pure SIP URI)
        originate_str = (
        ...
        # Execute the originate (synchronous, same as fs_cli -x)
        response = conn.api(originate_str)
        if response:
            # The response object has a getHeader method if it's an ESLevent
            if hasattr(response, 'getHeader'):
                reply = response.getHeader("Reply-Text")
            else:
                reply = str(response)
            ...
            return {"success": False, "error": "No response from FreeSWITCH"}
...
def check_call_status(call_uuid: str) -> dict:
    ...
    try:
        ...
        if not conn or not conn.connected():
            return {"success": False, "error": "ESL connection failed"}
        response = conn.api("show", f"channel {call_uuid}")
        status_text = response.getHeader("Reply-Text") if response else "Unknown"
        return {"success": True, "call_uuid": call_uuid, "status": status_text}
    except Exception as e:
