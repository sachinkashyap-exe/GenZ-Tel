    ...
    try:
        ...
        for i, line in enumerate(lines):
            if 'User:' in line and f'{agent_id}@' in line:
                found_user = True
                # The contact line usually appears after the User line
...
def get_agent_info(agent_id: str, db_session=None):
    """Get agent information from database (only existing columns)"""
    should_close_db = False
    if db_session is None:
        db_session = get_db_session()
        should_close_db = True
    try:
        result = db_session.execute(
            text("SELECT agent_id, full_name, extension, status, agent_type, allow_remote_login, agent_identity FROM agents WHERE agent_id = :agent_id"),
            {"agent_id": agent_id}
        )
        agent = result.first()
...
# ============================================
# Agent API Endpoints (UPDATED for new schema)
# ============================================

async def agent_login_endpoint(login_request):
    """Handle agent login (updated for new schema)"""
    db_session = get_db_session()
    try:
        app_logger.info(f"Login attempt: {login_request.agent_id}")

        # Fetch agent from DB - only existing columns
        result = db_session.execute(
            text("SELECT agent_id, full_name, extension, status, agent_type, allow_remote_login, password_hash FROM agents WHERE agent_id = :agent_id"),
        ...
        # Store in Redis (use agent_type instead of role)
        if REDIS_AVAILABLE and redis_client:
            redis_client.hset(f"agent:{login_request.agent_id}", mapping={
                "full_name": agent.full_name,
                "extension": agent.extension,
                "agent_type": agent.agent_type or "Normal",
                "status": "Available",
...
# ============================================
# Call Control Functions (unchanged, they work with new schema)
# ============================================
...
def originate_call_direct(agent_id: str, phone_number: str, caller_id: str = None) -> dict:
    ...
    try:
        ...
        # 1. Get the exact SIP contact URI
