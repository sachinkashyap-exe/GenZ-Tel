...
def check_call_status(call_uuid: str) -> dict:
    """
    Check live status of a call by UUID via ESL.
    Returns: {'success': bool, 'status': 'ringing'|'connected'|'ended'}
    """
    try:
        import ESL
        conn = ESL.ESLconnection("127.0.0.1", FS_PORT, FS_PASSWORD)
        if not conn or not conn.connected():
            return {"success": False, "error": "ESL connection failed", "status": "unknown"}

        # Use uuid_status for more accurate state
        response = conn.api("uuid_status", call_uuid)
        if not response:
            return {"success": False, "error": "No response from FreeSWITCH", "status": "unknown"}

        reply = response.getHeader("Reply-Text") or ""
        app_logger.info(f"uuid_status for {call_uuid}: {reply}")

        # Convert FreeSWITCH channel states to our simple states
        if "state=ACTIVE" in reply or "state=UP" in reply or "state=CONNECTED" in reply:
            status = "connected"
        elif "state=RINGING" in reply or "state=EARLY" in reply:
            status = "ringing"
        elif "state=HANGUP" in reply or "state=DOWN" in reply:
            status = "ended"
        else:
            # If uuid_status returns nothing, the call may not exist
            # Fallback to 'show channel' to see if it's still active
            show_resp = conn.api("show", f"channel {call_uuid}")
            show_reply = show_resp.getHeader("Reply-Text") if show_resp else ""
            if "not found" in show_reply.lower():
                status = "ended"
            else:
                # Maybe ringing or connected - assume connected if not found
                status = "connected"

        return {"success": True, "call_uuid": call_uuid, "status": status}
    except Exception as e:
        log_error("agent", e, {"function": "check_call_status"})
