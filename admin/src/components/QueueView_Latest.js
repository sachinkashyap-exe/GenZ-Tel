...
const QueueView = () => {
    ...
    const [applying, setApplying] = useState(false); // NEW: loading state for apply action
    ...
    // Apply to FreeSWITCH with loading indicator
    ...
    const styles = {
        container: { background: '#060b13', minHeight: '100vh', padding: '42px 40px 40px 40px', color: '#fff' }, // increased top padding
        header: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '30px', flexWrap: 'wrap', gap: '15px' },
        card: { background: '#0b121f', borderRadius: '20px', border: '1px solid #1e293b', overflow: 'auto' },
        table: { width: '100%', borderCollapse: 'collapse', minWidth: '800px' },
        th: { padding: '16px 20px', color: '#475569', fontSize: '11px', textTransform: 'uppercase', textAlign: 'left', borderBottom: '1px solid #1e293b' },
        td: { padding: '16px 20px', fontSize: '14px', borderBottom: '1px solid #1e293b' },
        statusBtn: (active) => ({
            background: active ? '#ef444420' : '#10b98120', color: active ? '#ef4444' : '#10b981',
            border: `1px solid ${active ? '#ef444440' : '#10b98140'}`, padding: '6px 12px', borderRadius: '8px', cursor: 'pointer', fontSize: '11px', fontWeight: 'bold', display: 'inline-flex', alignItems: 'center', gap: '6px'
        }),
        actionBtn: (disabled, color) => ({
            background: '#1e293b', color: disabled ? '#334155' : (color || '#94a3b8'), border: 'none', padding: '8px', borderRadius: '8px', cursor: disabled ? 'not-allowed' : 'pointer', marginLeft: '6px', opacity: disabled ? 0.3 : 1, transition: '0.2s'
        }),
        modalOverlay: { position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.8)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 2000, backdropFilter: 'blur(8px)' },
        modalBox: { background: '#0b121f', padding: '30px', borderRadius: '24px', width: '750px', maxWidth: '90vw', border: '1px solid #1e293b', maxHeight: '85vh', overflowY: 'auto' },
        input: { width: '100%', padding: '12px', background: '#060b13', border: '1px solid #1e293b', borderRadius: '12px', color: '#fff', marginTop: '6px', outline: 'none' }
    };

    if (loading) return <div style={{ ...styles.container, textAlign: 'center', paddingTop: '100px' }}>Loading campaigns...</div>;
        ...
        <div style={styles.container}>
            <div style={styles.header}>
                ...
                <div style={{ display: 'flex', gap: '15px' }}>
                    <div style={{ background: '#111827', border: '1px solid #1e293b', borderRadius: '12px', display: 'flex', alignItems: 'center', padding: '0 15px' }}>
                        ...
                        <input style={{ background: 'transparent', border: 'none', color: '#fff', padding: '10px', outline: 'none' }} placeholder="Search..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
                    </div>
                    <button style={{ background: '#0084ff', color: '#fff', border: 'none', padding: '10px 20px', borderRadius: '12px', cursor: 'pointer', fontWeight: 'bold' }} onClick={() => setShowAddForm(true)}>+ Add Campaign</button>
                </div>
            </div>
            ...
            <div style={styles.card}>
                <table style={styles.table}>
                    <thead>
                        <tr>
                            <th style={styles.th}>Customer</th><th style={styles.th}>Status</th><th style={styles.th}>Type</th><th style={styles.th}>Strategy</th><th style={styles.th}>Wrap-Up</th><th style={styles.th}>HB</th><th style={{ ...styles.th, textAlign: 'right' }}>Actions</th>
                        </tr>
                    </thead>
                    <tbody>
                        {filteredCampaigns.map(c => (
                            <tr key={c.id}>
                                <td style={styles.td}><b>{c.name}</b><br /><small style={{ color: '#475569' }}>{c.campaign_name}</small></td>
                                <td style={styles.td}>
                                    <button style={styles.statusBtn(c.is_active)} onClick={() => toggleActive(c)}>
                                        {c.is_active ? <XCircle size={14} /> : <CheckCircle2 size={14} />} {c.is_active ? 'Deactivate' : 'Activate'}
                                    </button>
                                </td>
            ...
            {/* ADD CAMPAIGN MODAL - with queue_timeout added */}
            {showAddForm && (
                <div style={styles.modalOverlay}>
                    <div style={styles.modalBox}>
                        <h3>New Campaign</h3>
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '20px' }}>
                            <div><label style={{ fontSize: '12px', color: '#475569' }}>Name *</label><input style={styles.input} value={newCampaign.name} onChange={e => setNewCampaign({ ...newCampaign, name: e.target.value })} /></div>
                            <div><label style={{ fontSize: '12px', color: '#475569' }}>Internal Name</label><input style={styles.input} value={newCampaign.campaign_name} onChange={e => setNewCampaign({ ...newCampaign, campaign_name: e.target.value })} /></div>
                            <div><label style={{ fontSize: '12px', color: '#475569' }}>Type</label>
                            ...
                            <div><label style={{ fontSize: '12px', color: '#475569' }}>Strategy</label>
                            ...
                            <div><label style={{ fontSize: '12px', color: '#475569' }}>Wrap-up (s)</label><input type="number" style={styles.input} value={newCampaign.dialer_wrapup_time} onChange={e => setNewCampaign({ ...newCampaign, dialer_wrapup_time: e.target.value })} /></div>
                            <div><label style={{ fontSize: '12px', color: '#475569' }}>Queue Timeout (s)</label><input type="number" style={styles.input} value={newCampaign.queue_timeout} onChange={e => setNewCampaign({ ...newCampaign, queue_timeout: e.target.value })} /></div>
                            <div><label style={{ fontSize: '12px', color: '#475569' }}>Heartbeat (s)</label><input type="number" style={styles.input} value={newCampaign.softphone_heartbeat} onChange={e => setNewCampaign({ ...newCampaign, softphone_heartbeat: e.target.value })} /></div>
            ...
            {/* EDIT CAMPAIGN MODAL - fixed overlapping */}
            {editModal.visible && (
                <div style={styles.modalOverlay}>
                    <div style={styles.modalBox}>
                        <h3>Edit Campaign: {editModal.campaign.name}</h3>
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '20px' }}>
                            <div><label>Name</label><input style={styles.input} value={editModal.formData.name} onChange={e => setEditModal({ ...editModal, formData: { ...editModal.formData, name: e.target.value } })} /></div>
                            <div><label>Internal Name</label><input style={styles.input} value={editModal.formData.campaign_name || ''} onChange={e => setEditModal({ ...editModal, formData: { ...editModal.formData, campaign_name: e.target.value } })} /></div>
                            ...
                            <div><label>Wrap-up (s)</label><input type="number" style={styles.input} value={editModal.formData.dialer_wrapup_time} onChange={e => setEditModal({ ...editModal, formData: { ...editModal.formData, dialer_wrapup_time: e.target.value } })} /></div>
                            <div><label>Queue Timeout (s)</label><input type="number" style={styles.input} value={editModal.formData.queue_timeout} onChange={e => setEditModal({ ...editModal, formData: { ...editModal.formData, queue_timeout: e.target.value } })} /></div>
                            <div><label>Heartbeat (s)</label><input type="number" style={styles.input} value={editModal.formData.softphone_heartbeat} onChange={e => setEditModal({ ...editModal, formData: { ...editModal.formData, softphone_heartbeat: e.target.value } })} /></div>
                            <div style={{ display: 'flex', gap: '20px', alignItems: 'center' }}>
            ...
            {ftpModal.visible && (
                <div style={styles.modalOverlay}>
                    <div style={{ ...styles.modalBox, width: '450px' }}>
                        ...
                        <div><label>Host</label><input style={styles.input} value={ftpModal.formData.host} onChange={e => setFtpModal({ ...ftpModal, formData: { ...ftpModal.formData, host: e.target.value } })} /></div>
                        <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '10px' }}>
                            <div><label>Username</label><input style={styles.input} value={ftpModal.formData.username} onChange={e => setFtpModal({ ...ftpModal, formData: { ...ftpModal.formData, username: e.target.value } })} /></div>
                            <div><label>Port</label><input style={styles.input} value={ftpModal.formData.port} onChange={e => setFtpModal({ ...ftpModal, formData: { ...ftpModal.formData, port: e.target.value } })} /></div>
                        </div>
                        <div><label>Password</label><input type="password" style={styles.input} value={ftpModal.formData.password} onChange={e => setFtpModal({ ...ftpModal, formData: { ...ftpModal.formData, password: e.target.value } })} /></div>
                        <div><label>Remote Path</label><input style={styles.input} value={ftpModal.formData.remote_path} onChange={e => setFtpModal({ ...ftpModal, formData: { ...ftpModal.formData, remote_path: e.target.value } })} /></div>
