...
import {
    ...
    fetchCampaigns, fetchAgentCampaigns, assignCampaignsToAgent   // added missing imports
} from '../services/adminService';
import { Search, Edit2, Trash2, UserPlus, RefreshCw, Users } from 'lucide-react';
...
const AgentsView = () => {
    ...
    const [assignModal, setAssignModal] = useState({ open: false, agent: null, selectedCampaigns: [] });
    ...
    // Open campaign assignment modal
    const openAssignModal = async (agent) => {
        try {
            const assigned = await fetchAgentCampaigns(agent.agent_id);
            const assignedIds = assigned.map(c => c.id);
            setAssignModal({
                open: true,
                agent: agent,
                selectedCampaigns: assignedIds
            });
        } catch (err) {
            alert('Failed to load campaign assignments: ' + (err.response?.data?.detail || err.message));
        }
    };

    const saveAssignments = async () => {
        try {
            await assignCampaignsToAgent(assignModal.agent.agent_id, assignModal.selectedCampaigns);
            alert('Campaigns assigned successfully');
            setAssignModal({ open: false, agent: null, selectedCampaigns: [] });
            loadAgents(); // refresh agent list (optional, but good)
        } catch (err) {
    ...
    return (
        <div style={styles.outerContainer}>
            <div style={styles.innerWrapper}>
                ...
                {assignModal.open && (
                    <div style={styles.modalOverlay} onClick={() => setAssignModal({ open: false, agent: null, selectedCampaigns: [] })}>
                        <div style={{ ...styles.modalBox, width: '500px' }} onClick={e => e.stopPropagation()}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                                <h3>Assign Campaigns to {assignModal.agent?.full_name}</h3>
                                <button
                                    onClick={() => setAssignModal({ open: false, agent: null, selectedCampaigns: [] })}
                                ...
                                {campaigns.map(camp => (
                                    <div key={camp.id} style={styles.campaignItem}>
                                        <input
                                            type="checkbox"
                                            id={`camp_${camp.id}`}
                                            checked={assignModal.selectedCampaigns.includes(camp.id)}
                                            onChange={(e) => {
                                                if (e.target.checked) {
                                                    setAssignModal(prev => ({
                                                        ...prev,
                                                        selectedCampaigns: [...prev.selectedCampaigns, camp.id]
                                                    }));
                                                } else {
                                                    setAssignModal(prev => ({
                                                        ...prev,
                                                        selectedCampaigns: prev.selectedCampaigns.filter(id => id !== camp.id)
                                                    }));
                                                }
                                            }}
                                        />
                                        <label htmlFor={`camp_${camp.id}`} style={{ fontSize: '14px', cursor: 'pointer' }}>
                                            {camp.name} ({camp.campaign_type})
                                        </label>
                                    </div>
                                ))}
                                {campaigns.length === 0 && (
                                    <div style={{ textAlign: 'center', color: '#475569', padding: '20px' }}>
                                        No campaigns available. Create campaigns first.
                                    </div>
                                )}
                            </div>
                            <div style={styles.buttonGroup}>
                                <button style={styles.buttonSecondary} onClick={() => setAssignModal({ open: false, agent: null, selectedCampaigns: [] })}>Cancel</button>
                                <button style={styles.buttonPrimary} onClick={saveAssignments}>Save Assignments</button>
                            </div>
                        </div>
                    </div>
                )}
